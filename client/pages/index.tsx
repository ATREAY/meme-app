...
const Home: FC = () => {
  ...
  const generate = async (description: string) => {
    setLoading(true);
    try {
      const response = await axios.post('http://localhost:3001/api/generate', { prompt: description }, { responseType: 'arraybuffer' });
    ...
    <>
      ...
      <main>
        ...
        <p className="w-2/3 mx-auto text-center my-10">
          Nextjs application that leverages the model trained by Stability AI and Runway ML to generate images using the
          Stable Diffusion Deep Learning model. The model can be found via github here:{' '}
        ...
        <section className="flex justify-center mt-20 w-2/3 mx-auto">
          {loading ? (
            <div role="status" className="w-full p-4 border border-gray-200 rounded shadow animate-pulse dark:border-gray-700">
            ...
            <>
              {imageDataURL && (
                <div className=" p-4 rounded flex justify-center w-full h-full">
                  <Image src={imageDataURL} alt="Generated Image" width={600} height={600} priority />
                </div>
              )}
            </>
          )}
